...
class Order(models.Model):
    ...
    transaction_id = models.CharField(max_length=20, verbose_name = "Nro. Transacción")
    ...
    price = models.ForeignKey(Price, on_delete = models.PROTECT, blank = True, null=True, verbose_name="Valores para e cálculo del Precio de la compra")

    # payment info
    payment_name = models.CharField(max_length=50, verbose_name = "Nombre del beneficiario", null = True, blank = True)
    payment_phone = models.CharField(max_length=20, verbose_name = "Teléfono")
    payment_email = models.EmailField(max_length=50, verbose_name = "Correo electrónico")
    payment_city = models.CharField(max_length=20, verbose_name = "Ciudad", help_text="Ciudad del banco de la tarjeta")
    ...
    @property
    def total(self): 
        if self.total_items >= self.price.min_quantity_amount:
            return self.total_items*(1-self.price.amunt_discount/100) + self.delivery_price
        else:
...
class OrderItem(models.Model):
    ...
    def total(self):
        ...
        if self.quantity >= price.min_quantity_whole:
            return self.quantity * self.price * (1-price.whole_discount/100)
        else:
